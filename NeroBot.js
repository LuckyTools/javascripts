/*
    Copyright (c) 2013-2014 by xTinneh 
    All be maked by xTinneh
    Do not modify or delete whitout my permission
*/

Array.prototype.isArray = true;
var tastyPlugShutDown;
if (typeof nerobotShutDown != 'undefined') tastyPlugShutDown();
(function(){
    var sock, afktime = Date.now(), pms = false, drag = false, hidevideo = false, joincd = false, cd = false,
    version = '1.3.6.1', commands = {}, tos = {}, boothcd = false, reconnect = true, hover = false,
    ...
    function socket() {
        function loadSocket() {
            ...
            sock.onopen = function() {
                reconint = 2;
                console.log('[NeroBot v' + version + '] Connected to socket!');
            ...
            sock.onmessage = function(data) {
                ...
                switch (data.z) {
                    ...
                    default:
                        console.log('NeroBot v' + version + '] Unknown socket command');
                }
            };
            sock.onclose = function() {
                console.log('[NeroBot v' + version + '] Disconnected from socket!');
    ...
    function startup() {
        ...
        if (room == '/nerobot') socket();
        Chat('init', 'NeroBot v' + version + ' now running!<br>Type /commands for a list of commands.');
        console.log('[NeroBot v' + version + '] Now running.');
    }
    ...
    tastyPlugShutDown = function() {
        ...
        saveSettings();
        if (sock) sock.close();
        console.log('[NeroBot v' + version + '] Shut down.');
    };
    ...
    function eventWLUpd() {
        ...
        if (settings.boothalert && API.getWaitListPosition() < settings.boothnotify && API.getWaitListPosition() != -1 && !boothcd) {
            ...
            boothcd = true;
            if (room == '/nerobot' && sock.readyState == 1) commands.cs()
        }
    }
    ...
    function eventCommand(a) {
        ...
        else if (room == '/nerobot' && sock && sock.readyState == 1) {
    ...
    commands.pm = function(a) {
        ...
        if (sock && sock.readyState != 1) return Chat('error', 'Not connected to NeroBot\'s server!');
        if (a.message == '/pm') return Chat('info', 'Usage: /pm @user message<br>Sends a private message to the user if they are using NeroBot or TastyPlug and you are each other\'s fans');
    ...
    commands.pmconnect = function() {
        if (room == '/nerobott' || (sock && sock.readyState == 1)) return;
        Chat('info', 'Connecting to NeroBot server');
        socket();
    };
    ...
    commands.commands = function() {
        if (room == '/nerobot') Chat('info', 'NeroBot commands: <a href="http://tastycat.net/tastybot/" target="_blank">Click Here</a>');
        Chat('info', 'NeroBot commands: ' + Object.keys(commands).join(', '));
    };
    ...
    commands.pmtoggle = function() {
        if (!sock || sock.readyState != 1) return Chat('error', 'Not connected to NeroBot\'s server!');
        togglepm = !togglepm;
        Chat('info', 'Receiving NeroBot PMs ' + (togglepm ? 'enabled!' : 'disabled!'));
    };
